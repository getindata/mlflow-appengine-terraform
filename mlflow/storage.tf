...
//  depends_on = [google_app_engine_application.mlflow_app]
  ...
  for_each   = toset(["serviceAccount:${var.snowflake_service_account}","serviceAccount:${var.project}@appspot.gserviceaccount.com", "serviceAccount:service-${data.google_project.project.number}@gae-api-prod.google.com.iam.gserviceaccount.com"])
  member     = each.key
}
